		...
		{	
			if(isset($_POST['destination']) && !empty($_POST['destination']))
			{
				if(isset($_POST['date']) && !empty($_POST['date']))
				{
					...
					$date=$_POST['date'];
					$query=mysqli_query($conn,("select ARRIVAL,DEPARTURE,DURATION,FLIGHT_CODE,AIRLINE_ID,PRICE_BUSINESS,PRICE_ECONOMY,PRICE_STUDENTS,PRICE_DIFFERENTLYABLED from flight where SOURCE='$source' && DESTINATION='$destination' && DATE='$date'"));
...
<head>
	...
	<style>
		*{
		...
		body{
			background-image:linear-gradient(rgba(0,0,0,0.5),rgba(0,0,0,0.5)), url(Images/s.jpg);
			height: 100vh;
